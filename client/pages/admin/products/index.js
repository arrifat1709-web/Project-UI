...
const Product = () => {
  ...
  const productUrl = {
    url: `https://project-sever.vercel.app/api/v1/product/?limit=10000&page=1&sort=-createdAt`,
  };

  //---------------------------------------------------------------catgories filter-----------------------------
  const handleFilterWithParantCategory = async (event) => {
    productUrl.url = `https://project-sever.vercel.app/api/v1/product/?limit=10000&page=1&category=${event.target.value}`;
    getFetchFunction(productUrl.url, setProducts);
  };

  const handleFilterWithParantCategory2 = async (event) => {
    productUrl.url = `https://project-sever.vercel.app/api/v1/product/?limit=10000&page=1&subCategory=${event.target.value}`;
    getFetchFunction(productUrl.url, setProducts);
  };
    ...
    <DashboardLayout>
      <div className="">
        ...
        <div className="p-8 rounded bg-white shadow flex justify-center items-center gap-4 flex-wrap md:flex-nowrap">
          ...
          <div className="w-full md:w-[20%]">
            <select
              onChange={handleFilterWithParantCategory2}
              className="select select-bordered w-full  focus:outline-none "
              placeholder="Category"
            >
              <option disabled selected>
